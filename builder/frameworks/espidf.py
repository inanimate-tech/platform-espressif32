...
def resolve_link_library_name(base_name, occurrence, link_names, used_names):
    ...
    whenever the link line carries it. Otherwise two rewrites that
    esp32-arduino-lib-builder's copy-libs.sh applies are tried: a ``_2`` suffix
    from its substring collision check, and the managed-component
    ``espressif__`` prefix it drops for components that are local to it. The
    plain name is kept when neither alternative is on the link line.
    """
    plain_name = base_name if occurrence == 1 else f"{base_name}_{occurrence}"
    if plain_name in link_names:
        return plain_name

    alternatives = [f"{plain_name}_2"]
    if plain_name.startswith("espressif__"):
        alternatives.append(plain_name[len("espressif__"):])
